...
import { useCreateBannerMutation } from "@/app/(admin)/services/api";
...
const Page = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      toast.error(
        error?.data?.message || error?.error || "Unable to create banner.",
      );
    }
  };
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Publish a banner with an image, schedule, and active state.
            </p>
          </div>
        </div>
